...
def get_features_markup(selected_set):
    ...
    standard_tags = ["Premium", "مهكر", "بدون إعلانات", "Pro", "مدفوع", "Mod" , "يجب تشغيل VPN", "يحوي بعض المشكلات", "تم اختراق السيرفر", "تحديث جديد" , "تمت اضافة ترجمة"]
...
def autofill_edit_callback(call):
    ...
    if action == 'af_edit_name':
        ...
        bot.send_message(call.message.chat.id, "الرجاء إرسال الوصف الجديد (باللغة العربية):")
...
def ask_english_mod_features(chat_id, user_id):
    ...
    bot.send_message(chat_id, "الرجاء إدخال ميزات التعديل باللغة الإنجليزية.", reply_markup=get_back_markup())
...
"""
    ...
    if 'english_mod_features' in data:
        confirmation_message += f"ميزات التعديل باللغة الإنجليزية: {data.get('english_mod_features')}\n"
...
def autofill_input_handler(message):
    ...
    try:
        ...
        # 4. Changes
        try:
            translated_changes = translator.translate(changes_en) if changes_en else "لا يوجد"
        except Exception as e:
        ...
        if temp['icon'].startswith('http'):
            try:
                ...
                if img_resp.status_code == 200:
                    ...
                    bot.delete_message(call.message.chat.id, call.message.message_id)
                    sent_msg = bot.send_photo(call.message.chat.id, photo_file, caption=f"✅ {temp['name']}")
                    user_data[user_id]['app_image'] = sent_msg.photo[-1].file_id
                else:
                    bot.send_message(call.message.chat.id, "⚠️ Failed to download icon. Please upload it manually later.")
            except Exception as e:
             ...
             bot.delete_message(call.message.chat.id, call.message.message_id)
             bot.send_photo(call.message.chat.id, temp['icon'], caption=f"✅ {temp['name']}")
...
def admin_approval_callback(call):
    ...
    if action == 'approve':
        ...
        if publish_target == 'arabic' or publish_target == 'both':
            ...
            source_str = 'مقترحات القناة' if data.get('source') == 'recommendation' else 'طلبات المشتركين'
            mod_label = 'معدلة وفيها' if data.get('app_type') == 'mod' else 'النسخة رسمية'
