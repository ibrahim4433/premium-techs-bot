...
def translate_description_callback(call):
    ...
    if call.data == 'translate_manual':
        ...
        # Perform auto translation (Synchronous for now to match TeleBot) 
        try:
            # We use nest_asyncio to call the async translator in a sync handler if needed, 
            # but googletrans 4.0.0-rc1 is async. 
            # For Vercel/webhooks with sync TeleBot, we might need a sync translator or run it in a loop.
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            translated = loop.run_until_complete(translator.translate(user_data[user_id]['app_description'], dest='en'))
